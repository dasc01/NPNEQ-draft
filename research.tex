...
\textsc{Inq} is the first implementation of density functional theory (DFT) and time-dependent DFT (TDDFT) that is written from scratch to work on graphical processing units (GPUs).

\textsc{Inq} achieves distributed memory parallelism (parallelism between several nodes) by using the Message Passing Interface (\textsc{MPI}) infrastructure, including extensions that allow direct GPU to GPU data transfer.
...
Unfortunately this last case has a non-negligible overhead in the communication cos
...
Having validated the modern C++ based design and functionality of \textsc{INQ} and demonstrated parallelism over hybrid  
